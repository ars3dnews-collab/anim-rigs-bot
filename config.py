...
# ---------------------------------------------------------------------------
# Запуск короткий: проверил время, опубликовал (или нет) и вышел.
#
# Длинные запуски со sleep внутри пробовали — отказались. Задание жило
# часами, а если залипало, то держало очередь и блокировало канал на
# полсуток, при этом формально «работая». Короткий запуск зависнуть не
# может: он живёт меньше минуты, а потолок в 20 минут добьёт даже
# невозможное. Ритм держит posted.json, а не живой процесс.
# ---------------------------------------------------------------------------
LOOP_MINUTES = 0
